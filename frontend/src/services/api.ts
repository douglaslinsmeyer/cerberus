...
// Reference to auth context (set by AuthProvider)
...
// Add response interceptor for error handling and 401 auto-refresh
api.interceptors.response.use(
  ...
  async (error) => {
    ...
    if (error.response?.status === 401 && !originalRequest._retry) {
      ...
      try {
        await authContextRef?.refreshAccessToken()
        const newToken = authContextRef?.accessToken
